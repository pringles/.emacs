...
(defun get-nsis-keywords ()
  (let ((result nil))
	(dolist (item (read-lines "~/.emacs.d/site-lisp/nsis.keywords"))
	  (if (keyword item) 
		   (setq result (cons item result)))) result))
...
nsi-beep-if-tab-change\t\tring the bell if `tab-width' is changed"
  ...
  (setq major-mode              'nsi-mode
	...
	font-lock-defaults      (list '(nsi-font-lock-keywords) nil t nil nil)
	paragraph-separate      "^[ \t]*$"
	paragraph-start         "^[ \t]*$"
...
subtleties, including the use of the optional ASYNC argument."
  ...
  (save-buffer)
  (shell-command (concat "C:/Progra~1/nsis/makensis.exe " nsi-file-name " &"))))
